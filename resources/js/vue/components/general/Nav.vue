<template>
    <nav>
        ...
        <div>
            <span class="flex">
                <p>Tyrian Time: {{ tyrianCurrentPeriod }}</p>
                <svg 
                    v-if="tyrianCurrentPeriod == 'Day'"
            ...
            <p>Tyian Time: {{ tyrianTimeTilNext }}</p>
        </div>
...
import { tyrianCurrentPeriod, tyrianTimeTilNext } from '@/js/vue/composables/Global.js';
...
const updateTimer = () => {
    ...
    const elapsedTime = (utcTime - startTime + 24 * 3600) % (2 * 3600); 
    ...
    for (const period of tyrianPeriods) {
        if (elapsedTime < totalDuration + period.duration) {
            currentPeriod = period.label;
            tyrianCurrentPeriod.value = currentPeriod;
            tyrianTimeTilNext.value = formatTime(period.duration - (elapsedTime - totalDuration));

            console.log('tyyian current peroid: ', tyrianCurrentPeriod.value);
            break;
        }
...
onMounted(() => {
    updateTimer(); 
    tyrianTimeInterval = setInterval(updateTimer, 1000);
})

onUnmounted(() => {
    clearInterval(tyrianTimeInterval);
})
