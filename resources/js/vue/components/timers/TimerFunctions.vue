    ...
    <article>
        <div class="icons">
            ...
            <svg 
                ...
                @click="toggleInfo(index); scrollTo(`timer-${event.name}`)"
            >
...
import { store, share } from '@/js/vue/composables/TimerFunctions.js'
...
store.setTogglePlay(props.index, {status: true, outpost: props.event.outpost});
store.setToggleInfo(props.index, false);

const toggleTimer = (index) => {
    store.setTogglePlay(index, {status: !store.togglePlay[index].status});
}

const toggleInfo = (index) => {
    store.setToggleInfo(index, !store.toggleInfo[index]);
}
