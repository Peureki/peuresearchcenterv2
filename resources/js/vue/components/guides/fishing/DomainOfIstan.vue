<template>
    <div class="guide-container">
        <h3>Guide</h3>

        <GuideItem
            v-for="(guide, index) in guides"
            :index="index + 1"
            :instruction="guide.instruction"
            :waypoint-name="guide.waypointName"
            :waypoint-link="guide.waypointLink"
            :img="guide.img"
            :alt="guide.alt"
            @handle-show-tooltip="showTooltip"
        />       
    </div>

    <Transition name="fade">
        <CursorTooltip v-if="tooltipToggle" message="Copied!" :mouseX="mouseX" :mouseY="mouseY" />
    </Transition>
</template>

<script setup>
import { ref } from 'vue'
import { handleCursorTooltip } from '@/js/vue/composables/MouseFunctions'

// Cursor tooltip
import CursorTooltip from '@/js/vue/components/general/CursorTooltip.vue';
import GuideItem from '@/js/vue/components/guides/nodes/GuideItem.vue';

// Picture guides
import OffshoreFish from '@/imgs/guides/fishing/Domain_of_Istan_Offshore_Fish.webp'

// Initilize tooltip vars
const { mouseX, mouseY, tooltipToggle, showTooltip } = handleCursorTooltip(); 

// *
// * FILL IN GUIDES AND PICTURES
// *
const guides = ref([
    {
        instruction: "From the waypoint, follow the water all the way down the south edge then west. Once you get to the bay, pools will be so far apart that it's best to waypoint back.",
        waypointName: "Chalon Docks Waypoint",
        waypointLink: "[&BAkLAAA=]",
    },
    {
        instruction: "Alternatively, you could go from Astralarium and follow the water east, then south, then west the whole way. When you can't personl waypoint back, go back to #1.",
        waypointName: "Astralarium Waypoint",
        waypointLink: "[&BPoKAAA=]",
        img: OffshoreFish,
        alt: "Offshore Fish Guide",
    },
])

</script>